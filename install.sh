...
echo -e "${CYAN}[*] Installing DroidX v1.0...${RESET}"
...
curl -sL "https://raw.githubusercontent.com/AdhiHub/droidx/main/droidx.sh" -o /usr/local/bin/droidx.sh 2>/dev/null || {
    echo -e "${RED}[!] Download failed. Check internet connection.${RESET}"
    exit 1
}

chmod +x /usr/local/bin/droidx.sh

echo -e "${GREEN}[✓] DroidX installed successfully!${RESET}"
echo -e "${CYAN}Run: droidx.sh${RESET}"
echo ""
